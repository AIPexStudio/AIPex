...
class ZenFSManager {
     ...
     */
    async _readDirRecursive(basePath, currentPath, files) {
     ...
     */
    async getFileTree(basePath = "/skills") {
        await this.ensureInitialized();
     ...
     */
    async _buildFileTree(currentPath) {
        const entries = await fs.promises.readdir(currentPath);
        const nodes = [];
        for (const entry of entries) {
            if (entry === "." || entry === "..")
     ...
     */
    async getDiskUsage(basePath = "/skills") {
        await this.ensureInitialized();
        const usage = {
            totalSize: 0,
            fileCount: 0,
     ...
     */
    async _calculateDiskUsage(currentPath, usage, basePath) {
        const entries = await fs.promises.readdir(currentPath);
        for (const entry of entries) {
            if (entry === "." || entry === "..")
                continue;
     ...
     */
    async _calculateSkillUsage(skillPath, skillUsage) {
