...
class QuickJSManager {
  ...
  initPromise = null;
  initialized = false;
  moduleCache = new Map(); // Cache for CDN modules
  /**
   ...
   */
  async fetchESM(url, visited = new Set()) {
   ...
   */
  async execute(code, context, apiBridge) {
...
(function() {
   ...
   */
  safeDispose(vm, handle) {
    if (!this.isBuiltInConstant(vm, handle)) {
      handle.dispose();
    }
  }
  bindObject(vm, target, name = null) {
   ...
   */
  _injectGlobalAPI(vm, apiBridge, scope) {
