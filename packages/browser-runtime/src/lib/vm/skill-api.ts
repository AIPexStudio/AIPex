...
export interface SkillAPIBridge {
  ...
  // Browser Downloads
  downloadFile(data: string, options?: { filename?: string; encoding?: 'base64' | 'utf8'; saveAs?: boolean }): Promise<{ success: boolean; downloadId?: number; error?: string }>
  ...
  return {
    ...
    fs: {
      async readFile(path: string, encoding?: string): Promise<string | Uint8Array> {
        ...
        if (!encoding && result && typeof result === 'object' && !(result instanceof Uint8Array)) {
    ...
    async downloadFile(
      data: string,
      options?: { filename?: string; encoding?: 'base64' | 'utf8'; saveAs?: boolean }
