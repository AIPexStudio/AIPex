...
export const captureScreenshotTool = tool({
    ...
    description: "Capture screenshot of current visible tab and return as base64 data URL. When sendToLLM=true, the screenshot will be sent to the LLM for visual analysis AND visual coordinate tools (computer) will be unlocked for subsequent interactions.",
    parameters: z.object({
        sendToLLM: z
            .boolean()
            .nullable()
            .optional()
            .default(false)
            .describe("Whether to send the screenshot to LLM for visual analysis. When true, visual coordinate tools will be enabled."),
    }),
    execute: async ({ sendToLLM = false }) => {
        ...
        if (sendToLLM) {
            ...
            if (viewport) {
                cacheScreenshotMetadata(tab.id, img.width, img.height, viewport.width, viewport.height);
            }
        }
        return {
            success: true,
...
export const captureTabScreenshotTool = tool({
    ...
    description: "Capture screenshot of a specific tab by ID. When sendToLLM=true, the screenshot will be sent to the LLM for visual analysis AND visual coordinate tools (computer) will be unlocked for subsequent interactions.",
    parameters: z.object({
    ...
    execute: async ({ tabId, sendToLLM = false, }) => {
        ...
        if (sendToLLM) {
            ...
            // Cache screenshot metadata for computer tool
            if (viewport) {
                cacheScreenshotMetadata(tabId, img.width, img.height, viewport.width, viewport.height);
            }
        }
        return {
            success: true,
