 ...
 */
export async function downloadTextAsMarkdown(text, filename) {
    try {
        if (!chrome.downloads) {
            return {
                success: false,
...
export const downloadImageTool = tool({
    name: "download_image",
    description: "Download an image from base64 data to the user's local filesystem",
    parameters: z.object({
        imageData: z
            .string()
            .regex(/^data:image\//)
...
export const downloadChatImagesTool = tool({
    ...
    execute: async ({ messages, folderPrefix, filenamingStrategy = "descriptive", }) => {
        try {
            if (!chrome.downloads) {
                return {
                    success: false,
                ...
                for (const part of message.parts) {
                    if (part.type === "image" && part.imageData) {
                        try {
                            ...
                            errors.push(`Failed to download image ${imageIndex}: ${error instanceof Error ? error.message : String(error)}`);
                        }
                    }
                }
            }
...
export const downloadCurrentChatImagesTool = tool({
    ...
    execute: async ({ folderPrefix: _folderPrefix, }) => {
        ...
        return {
            success: false,
            message: "This tool requires integration with the chat system to access current conversation images",
        };
    },
});
