...
export const listInterventionsTool = tool({
    ...
    description: "List all available human intervention types. Use this to discover what interventions the AI can request from the user.",
    parameters: z.object({
        enabledOnly: z
            .boolean()
            .default(false)
        ...
        try {
            ...
            if (currentMode === "disabled") {
                return {
                    ...
                    message: "Current conversation settings do not allow any human intervention features. User has explicitly forbidden AI from requesting interventions.",
                };
            }
            // Ensure initialized
...
export const getInterventionInfoTool = tool({
    ...
    description: "Get detailed information about a specific intervention type, including input/output schemas and examples.",
    parameters: z.object({
        type: z
            .enum(["monitor-operation", "voice-input", "user-selection"])
            .describe("Intervention type (monitor-operation, voice-input, user-selection)"),
    }),
    execute: async ({ type }) => {
        try {
            // Check intervention mode
            ...
            const metadata = interventionRegistry.getMetadata(type);
            if (!metadata) {
                return {
                    success: false,
...
export const requestInterventionTool = tool({
    ...
    description: "Request a human intervention. This will pause AI execution and wait for user input. Use this when you need the user to perform an action or provide information that cannot be obtained programmatically.",
    parameters: z.object({
        type: z
            .enum(["monitor-operation", "voice-input", "user-selection"])
            .describe("Intervention type (monitor-operation, voice-input, user-selection)"),
        params: z
            ...
            .nullable()
            .default({})
            .describe("Intervention-specific parameters. See get_intervention_info for details."),
        timeout: z
            .number()
            .default(300)
        ...
        try {
            ...
            if (currentMode === "disabled") {
                return {
                    success: false,
                    error: "🚫 Human intervention features are disabled",
                    message: "Current conversation settings do not allow any human intervention features. User has explicitly forbidden AI from requesting interventions. Please do not try to call any intervention tools.",
                    status: "cancelled",
                };
            }
            // Execute intervention
            const result = await interventionManager.requestIntervention(type, params && Object.keys(params).length > 0 ? params : undefined, timeout, reason || undefined);
            if (result.success) {
                return {
                    success: true,
                    result: result.data,
...
export const cancelInterventionTool = tool({
    ...
    description: "Cancel the currently active intervention. Use this if you realize the intervention is no longer needed.",
    parameters: z.object({
        id: z
            .string()
            .nullable()
            .default(null)
            .describe("Intervention ID (optional, will cancel current if not provided)"),
    }),
    execute: async ({ id }) => {
        try {
            // Ensure initialized
            ...
            const cancelled = interventionManager.cancelIntervention(currentIntervention.request.id);
            if (cancelled) {
                return {
                    success: true,
