...
import { playClickAnimationAndReturn, scrollAndMoveFakeMouseToElement, waitForEventsAfterAction, } from "./ui-operations";
...
export const clickTool = tool({
    ...
    execute: async ({ tabId, uid, dblClick = false, }) => {
        ...
        try {
            ...
            if (!handle) {
                throw new Error("Element not found in current snapshot. Call take_snapshot first.");
            }
...
export const getEditorValueTool = tool({
    ...
    description: "Get the complete content from a code editor (Monaco, CodeMirror, ACE) or textarea without truncation. Use this before filling to avoid data loss.",
    parameters: z.object({
...
export const fillFormTool = tool({
    name: "fill_form",
    description: "Fill multiple form elements at once using their UIDs from a snapshot",
    parameters: z.object({
    ...
    execute: async ({ tabId, elements, }) => {
            ...
            try {
                ...
                if (!handle) {
                    results.push({
                        uid: element.uid,
                        success: false,
                        error: "Element not found in current snapshot. Call take_snapshot first.",
                    });
                    continue;
                }
        ...
        return {
            ...
            results,
            message: successCount === elements.length
