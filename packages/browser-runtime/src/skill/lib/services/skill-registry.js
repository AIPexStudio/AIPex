...
import { getSkillAssets, getSkillReferences, getSkillScripts, } from "../utils/zip-utils";
export class SkillRegistry {
    skills = new Map();
    initialized = false;
    async initialize(skillMetadataList) {
        if (this.initialized)
            return;
        ...
        for (const metadata of skillMetadataList) {
            ...
            try {
                ...
                if (metadata.enabled) {
                    ...
                    skillMdContent =
                        (await skillStorage.getSkillFile(metadata.id, "SKILL.md")) || "";
    ...
    async getSkillReference(skillName, refPath) {
        ...
        const content = (await skillStorage.getSkillFile(skill.metadata.id, refPath));
        if (!content) {
    ...
    async getSkillScript(skillName, scriptPath) {
        ...
        const content = (await skillStorage.getSkillFile(skill.metadata.id, scriptPath));
        if (!content) {
    ...
    updateSkillStatus(skillName, enabled) {
        const existing = this.skills.get(skillName);
        if (!existing) {
            return false;
        }
