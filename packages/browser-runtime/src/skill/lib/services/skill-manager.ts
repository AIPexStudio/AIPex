import { skillStorage, type SkillMetadata, type ParsedSkill } from '../storage/skill-storage'
...
export class SkillManager {
  ...
  // --- Event System ---
  public subscribe(event: SkillEventType, callback: SkillSubscriber): () => void {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      this._emit('skill_loaded', { type: 'initialization_complete', skills: updatedSkills })
    } catch (error) {
  ...
  async uploadSkill(zipFile: File, replace: boolean = false): Promise<SkillMetadata> {
  ...
  async executeSkillScript(skillName: string, scriptPath: string, args: any = {}): Promise<any> {
    if (!this.initialized) {
    ...
    try {
      ...
      console.log(`✅ Script executed successfully: ${skillName}/${scriptPath}`)
      return result
    } catch (error) {
      console.error(`❌ Failed to execute script ${skillName}/${scriptPath}:`, error)
      throw error
    }
  }
  ...
  async getSkillAsset(skillName: string, assetPath: string): Promise<string | ArrayBuffer | null> {
    if (!this.initialized) {
      throw new Error('SkillManager not initialized')
    }
  ...
  private async loadBuiltinUxAuditWalkthrough(): Promise<void> {
    try {
      ...
      if (!skillMdExists) {
        ...
        console.log(`✅ UX-audit-walkthrough files written to ZenFS: ${skillPath}`)
      } else {
        console.log('✅ Built-in ux-audit-walkthrough SKILL.md already exists in ZenFS')
      }
      ...
      if (!existingMetadata) {
        ...
        console.log('✅ Built-in ux-audit-walkthrough metadata already exists in IndexedDB')
      }

      console.log('✅ Built-in ux-audit-walkthrough loaded successfully (disabled by default)')
    } catch (error) {
  ...
  private async loadBuiltinWcag22A11yAudit(): Promise<void> {
    try {
      ...
      if (!skillMdExists) {
        ...
        console.log(`✅ wcag22-a11y-audit files written to ZenFS: ${skillPath}`)
      } else {
        console.log('✅ Built-in wcag22-a11y-audit SKILL.md already exists in ZenFS')
      }
      ...
      if (!existingMetadata) {
        ...
        console.log('✅ Built-in wcag22-a11y-audit metadata already exists in IndexedDB')
      }
