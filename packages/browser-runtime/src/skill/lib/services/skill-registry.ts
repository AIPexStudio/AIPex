import { type SkillMetadata, type ParsedSkill, skillStorage } from '../storage/skill-storage'
import { getSkillScripts, getSkillReferences, getSkillAssets } from '../utils/zip-utils'
...
export class SkillRegistry {
  ...
  async initialize(skillMetadataList: SkillMetadata[]): Promise<void> {
    ...
    for (const metadata of skillMetadataList) {
      console.log(`Processing skill: ${metadata.name} (enabled: ${metadata.enabled})`)
      try {
        ...
        if (metadata.enabled) {
          ...
          skillMdContent = await skillStorage.getSkillFile(metadata.id, 'SKILL.md') as string || ''
        ...
        console.log(`✅ Successfully loaded skill: ${metadata.name} (enabled: ${metadata.enabled})`)
      } catch (error) {
  ...
  async getSkillReference(skillName: string, refPath: string): Promise<string> {
    ...
    const content = await skillStorage.getSkillFile(skill.metadata.id, refPath) as string
    if (!content) {
  ...
  async getSkillScript(skillName: string, scriptPath: string): Promise<string> {
    ...
    const content = await skillStorage.getSkillFile(skill.metadata.id, scriptPath) as string
    if (!content) {
  ...
  async getSkillAsset(skillName: string, assetPath: string): Promise<string | ArrayBuffer | null> {
    ...
    const content = await skillStorage.getSkillFile(skill.metadata.id, assetPath)
    return content
  }
