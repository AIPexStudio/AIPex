...
export const SkillCard: React.FC<SkillCardProps> = ({
  ...
  const [fileStats, setFileStats] = useState({ scripts: 0, references: 0, assets: 0 })
  ...
  // Load file statistics from ZenFS
  useEffect(() => {
    const loadFileStats = async () => {
      try {
        const skillPath = zenfs.getSkillPath(skill.id)

        const [scriptsFiles, referencesFiles, assetsFiles] = await Promise.all([
          getSkillScripts(skillPath),
          getSkillReferences(skillPath),
          getSkillAssets(skillPath)
        ])

        setFileStats({
          scripts: scriptsFiles.length,
          references: referencesFiles.length,
          assets: assetsFiles.length
        })
      } catch (error) {
        console.error(`Failed to load file stats for skill ${skill.id}:`, error)
