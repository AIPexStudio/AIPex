...
export async function takeSnapshot() {
    ...
    try {
        ...
        console.log(`✅ [DEBUG] Snapshot preview:\n${snapshotText.split("\n").slice(0, 20).join("\n")}`);
        return {
            success: true,
            snapshotId: tab.id,
...
async function checkTabValid(tabId) {
 ...
 */
export async function getElementByUid(tabId, uid) {
    if (!(await checkTabValid(tabId))) {
        throw new Error("No accessible tab found");
    }
    const node = snapshotManager.getNodeByUid(tabId, uid);
    if (!node) {
        throw new Error("No such element found in the snapshot, the page content may have changed, please call search_elements again to get a fresh snapshot");
    }
 ...
 */
async function waitForEventsAfterAction(action) {
    // Execute the action
    await action();
...
export async function clickElementByUid(params) {
    ...
    try {
        ...
        if (!handle) {
            return {
                success: false,
                message: "Element not found in current snapshot. Call search_elements first to get fresh element UIDs.",
            };
        }
...
export async function fillElementByUid(params) {
    ...
    let handle = null;
    try {
        console.log("🔍 [DEBUG] Starting fillElementByUid using new Locator system for uid:", uid);
        // Step 1: Get element handle using snapshot UID mapping
        handle = await getElementByUid(tabId, uid);
        if (!handle) {
            return {
                success: false,
                message: "Element not found in current snapshot, the page content may have changed, please call search_elements again to get a fresh snapshot.",
            };
        }
...
export async function fillForm(params) {
    ...
    try {
        console.log("🔍 [DEBUG] Starting fillForm using new Locator system for", elements.length, "elements");
        let successCount = 0;
        const errors = [];
        for (const element of elements) {
            let handle = null;
            try {
                console.log(`🔍 [DEBUG] Processing element UID: ${element.uid} with value: "${element.value}"`);
                    ...
                    if (rectBeforeScroll) {
                        ...
                        if (rectAfterScroll) {
                            ...
                            if (finalRect) {
                                ...
                                const targetX = elementCenterX + cursorTipOffsetX;
                                const targetY = elementCenterY + cursorTipOffsetY;
                                console.log(`[UI Operations] Moving fake mouse to form field ${element.uid}:`, {
                                    center: { x: elementCenterX, y: elementCenterY },
                                    target: { x: targetX, y: targetY },
                                });
                                await chrome.tabs
                ...
                console.log(`✅ [DEBUG] Successfully filled element UID: ${element.uid}`);
                successCount++;
            }
            catch (error) {
...
export async function hoverElementByUid(params) {
    ...
    let handle = null;
    try {
        console.log("🔍 [DEBUG] Starting hoverElementByUid using new Locator system for uid:", uid);
        // Step 1: Get element handle using snapshot UID mapping
        handle = await getElementByUid(tabId, uid);
        if (!handle) {
            return {
...
export async function searchSnapshotText(params) {
    ...
    const result = await snapshotManager.searchAndFormat(tabId, query, contextLevels);
    if (!result) {
        return {
            success: false,
...
export async function getEditorValueByUid(params) {
    ...
    try {
        ...
        if (value === null) {
            return {
                success: false,
                message: "Failed to get editor value - element may not be an input/textarea/editor",
            };
        }
        console.log(`✅ [DEBUG] Successfully retrieved editor value (${value.length} characters)`);
        return {
            success: true,
