    ...
    async ensureNoExtensionFrame(tabId) {
        const result = await chrome.scripting.executeScript({
            ...
            func: () => {
                ...
                const extensionFrames = frames?.filter((frame) => frame.src.startsWith("chrome-extension://"));
        ...
        const attachPromise = new Promise((resolve) => {
            ...
            chrome.debugger.attach({ tabId }, "1.3", () => {
                if (chrome.runtime.lastError) {
                    console.error("❌ [DEBUG] Failed to attach debugger:", chrome.runtime.lastError.message);
                    resolve(false);
                }
                else {
