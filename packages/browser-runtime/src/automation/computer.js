 ...
 */
function logCdp(tabId, action, command, params, extra) {
    // Redact sensitive fields
    const safeParams = { ...params };
    if ("text" in safeParams && typeof safeParams.text === "string") {
        safeParams.text = `[REDACTED: ${safeParams.text.length} chars]`;
    }
    const logData = {
        tabId,
        action,
    ...
    console.log(`🔧 [CDP] chrome.debugger.sendCommand:`, JSON.stringify(logData, null, 2));
}
 ...
 */
export function cacheScreenshotMetadata(tabId, imageWidth, imageHeight, viewportWidth, viewportHeight) {
    const metadata = {
        imageWidth,
        imageHeight,
        viewportWidth,
    ...
    console.log(`📸 [Computer] Screenshot metadata cached for tab ${tabId}: ${imageWidth}x${imageHeight} image, ${viewportWidth}x${viewportHeight} viewport`);
}
/**
 ...
 */
function screenshotToCssPixels(x, y, metadata) {
 ...
 */
async function performClick(tabId, x, y, button, clickCount = 1) {
    const metadata = screenshotCache.get(tabId);
    if (!metadata) {
        return {
            success: false,
            error: "No screenshot metadata found. Please take a screenshot first using capture_screenshot(sendToLLM=true).",
        };
    }
    if (Date.now() - metadata.timestamp > 5 * 60 * 1000) {
        console.warn("⚠️ [Computer] Screenshot metadata is stale, coordinates may be inaccurate");
    }
    const { xCss, yCss } = screenshotToCssPixels(x, y, metadata);
    console.log(`🖱️ [Computer] ${button}_click at screenshot (${x},${y}) -> CSS (${xCss},${yCss})`);
    try {
        ...
        for (let i = 0; i < clickCount; i++) {
            ...
            logCdp(tabId, `${button}_click`, "Input.dispatchMouseEvent", pressedParams, {
                screenshotCoords: { x, y },
                cssCoords: { xCss, yCss },
            });
            ...
            logCdp(tabId, `${button}_click`, "Input.dispatchMouseEvent", releasedParams, {
                screenshotCoords: { x, y },
                cssCoords: { xCss, yCss },
            });
 ...
 */
async function performHover(tabId, x, y) {
    const metadata = screenshotCache.get(tabId);
    if (!metadata) {
        return {
            success: false,
    ...
    const { xCss, yCss } = screenshotToCssPixels(x, y, metadata);
    console.log(`🖱️ [Computer] Hover at screenshot (${x},${y}) -> CSS (${xCss},${yCss})`);
    try {
        const attached = await debuggerManager.safeAttachDebugger(tabId);
        if (!attached) {
            return { success: false, error: "Failed to attach debugger" };
 ...
 * Perform drag action
 */
async function performDrag(tabId, startX, startY, endX, endY) {
    const metadata = screenshotCache.get(tabId);
    if (!metadata) {
        return {
            success: false,
    ...
    console.log(`🖱️ [Computer] Drag from (${start.xCss},${start.yCss}) to (${end.xCss},${end.yCss})`);
    try {
        const attached = await debuggerManager.safeAttachDebugger(tabId);
        if (!attached) {
            return { success: false, error: "Failed to attach debugger" };
        ...
        logCdp(tabId, "left_click_drag", "Input.dispatchMouseEvent", pressedParams, {
        ...
        logCdp(tabId, "left_click_drag", "Input.dispatchMouseEvent", releasedParams, {
 ...
 */
async function performScroll(tabId, x, y, direction, amount) {
    const metadata = screenshotCache.get(tabId);
    if (!metadata) {
        return {
            success: false,
    ...
    console.log(`📜 [Computer] Scroll ${direction} by ${scrollAmount} at (${xCss},${yCss})`);
    try {
        const attached = await debuggerManager.safeAttachDebugger(tabId);
        if (!attached) {
            return { success: false, error: "Failed to attach debugger" };
 ...
 * Press key(s)
 */
async function performKey(tabId, keySequence) {
    ...
    try {
        ...
        const isMac = platformResult?.result?.value ===
            true;
 ...
 */
async function performScrollTo(tabId, uid) {
 ...
 */
export async function executeComputerAction(params) {
    ...
    if (!tabId && action !== "wait") {
        return {
            success: false,
            error: "No accessible tab found. Please specify a valid tabId or ensure there is an active tab.",
        };
    }
    switch (action) {
        case "left_click":
            ...
            return performClick(tabId, params.coordinate[0], params.coordinate[1], "left", 1);
        case "right_click":
            if (!params.coordinate || params.coordinate.length !== 2) {
                return {
                    success: false,
            ...
            return performClick(tabId, params.coordinate[0], params.coordinate[1], "right", 1);
        case "double_click":
            if (!params.coordinate || params.coordinate.length !== 2) {
                return {
                    success: false,
            ...
            return performClick(tabId, params.coordinate[0], params.coordinate[1], "left", 2);
        case "triple_click":
            if (!params.coordinate || params.coordinate.length !== 2) {
                return {
                    success: false,
            ...
            return performClick(tabId, params.coordinate[0], params.coordinate[1], "left", 3);
        case "hover":
            if (!params.coordinate || params.coordinate.length !== 2) {
                return {
                    success: false,
        ...
        case "left_click_drag":
            if (!params.start_coordinate || params.start_coordinate.length !== 2) {
                return {
                    success: false,
                    error: "start_coordinate parameter required for left_click_drag action",
                };
            }
            if (!params.coordinate || params.coordinate.length !== 2) {
                return {
                    success: false,
                    error: "coordinate parameter required for left_click_drag action (end position)",
                };
            }
            return performDrag(tabId, params.start_coordinate[0], params.start_coordinate[1], params.coordinate[0], params.coordinate[1]);
        case "scroll":
            if (!params.coordinate || params.coordinate.length !== 2) {
                return {
                    success: false,
            ...
            return performScroll(tabId, params.coordinate[0], params.coordinate[1], params.scroll_direction, params.scroll_amount);
        case "scroll_to":
            if (!params.uid) {
                return {
                    success: false,
 ...
 */
export function clearScreenshotCache(tabId) {
    if (tabId !== undefined) {
 ...
 */
export function getScreenshotMetadata(tabId) {
