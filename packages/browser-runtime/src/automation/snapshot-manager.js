...
import { SKIP_ROLES, searchSnapshotText } from "./query";
/**
 * 快照管理器
 *
 * 负责创建、管理和格式化页面快照
...
export class SnapshotManager {
     ...
     */
    async fetchExistingNodeIds(tabId, nodeMap) {
        console.log("🔍 [DEBUG] Fetching existing aipex-nodeids and tagNames from page");
        ...
        try {
            ...
            if (!attached) {
                console.warn("⚠️ [DEBUG] Failed to attach debugger for fetching existing IDs and tagNames");
                return existingData;
            }
            // Enable DOM domain
            ...
            const fetchTasks = Array.from(nodeMap.values())
                ...
                return limit(async () => {
                    try {
                        ...
                        const resolved = await cdpCommander.sendCommand("DOM.resolveNode", {
                        ...
                        const result = await cdpCommander.sendCommand("Runtime.callFunctionOn", {
            ...
            console.log(`✅ [DEBUG] Found ${existingData.size} existing aipex-nodeids with tagNames`);
            // Disable DOM domain
     ...
     */
    async getRealAccessibilityTree(tabId) {
        try {
            ...
            const result = await cdpCommander.sendCommand("Accessibility.getFullAXTree", {
     ...
     */
    hasInterestingDescendantsInSet(axNode, interestingNodes, nodeMap) {
        if (!axNode.childIds) {
            return false;
        }
        for (const childId of axNode.childIds) {
            ...
            if (childNode &&
                this.hasInterestingDescendantsInSet(childNode, interestingNodes, nodeMap)) {
                return true;
            }
        }
        return false;
     ...
     */
    isInterestingNode(axNode, insideControl = false) {
        const role = axNode.role?.value || "";
        const name = axNode.name?.value || "";
        const value = typeof axNode.value?.value === "string" ? axNode.value.value : "";
        const description = typeof axNode.description?.value === "string"
        ...
        if (layoutRoles.includes(role)) {
            ...
            const hasContent = [name, value, description].some((content) => content && content.trim().length > 1);
            return hasContent;
        }
    ...
    collectInterestingNodes(params) {
    ...
    serializeTree(params) {
        const { axNode, interestingNodes, nodeMap, idToNode, existingNodeData } = params;
        ...
        if (role === "link" && name) {
            ...
            if (urlMatch) {
                ...
                if (firstHalf === secondHalf && firstHalf.length > 0) {
                    ...
                    name = `${firstHalf} ${url}`;
                    console.log(`🔧 [DEBUG] Normalized duplicated link name: "${axNode.name?.value}" → "${name}"`);
                }
     ...
     */
    convertAccessibilityTreeToSnapshot(snapshotResult, existingNodeData) {
        ...
        console.log("📊 [DEBUG] Role distribution:", Array.from(roleCounts.entries())
            .sort((a, b) => b[1] - a[1])
            .slice(0, 10)
        ...
        const interestingNodes = new Set(); // Store nodeIds
        ...
        const finalInterestingNodes = new Set();
        for (const nodeId of interestingNodes) {
            ...
            if (node) {
                ...
                if (role === "generic" && !name && !value && !description) {
                    ...
                    const hasInterestingDescendants = this.hasInterestingDescendantsInSet(node, interestingNodes, nodeMap);
                    if (!hasInterestingDescendants) {
                        console.log(`  ✗ Filtered out pure layout container: ${role} "${name}"`);
                        continue;
                    }
                }
        ...
        console.log(`✅ [DEBUG] After filtering: ${finalInterestingNodes.size} truly interesting nodes`);
    ...
    async createSnapshot(tabId) {
        try {
            ...
            const snapshotResult = this.convertAccessibilityTreeToSnapshot(axTree, existingNodeData);
            if (!snapshotResult) {
            ...
            await this.injectNodeIdsToPage(tabId, snapshot.idToNode, existingNodeData);
     ...
     */
    async injectNodeIdsToPage(tabId, idToNode, existingNodeData) {
        ...
        try {
            ...
            const injectTasks = Array.from(idToNode.entries()).map(([uid, node]) => {
                ...
                return limit(async () => {
                    try {
                        ...
                        const resolved = await cdpCommander.sendCommand("DOM.resolveNode", { backendNodeId: node.backendDOMNodeId });
                        if (!resolved?.object?.objectId) {
            ...
            console.log(`✅ [DEBUG] Node injection complete: ${successCount} injected, ${skippedCount} skipped (already set), ${failedCount} failed`);
            // disable DOM domains
            await cdpCommander.sendCommand("DOM.disable", {});
    ...
    formatSnapshot(snapshot) {
        ...
        for (const [id, node] of snapshot.idToNode.entries()) {
            if (node.focused)
                focusedNodeIds.push(id);
        }
        // 计算所有焦点祖先链（把祖先都标记为 focus-path）
        const focusAncestorSet = new Set();
        // helper: DFS to find path from root to target
        function findPath(rootIdLocal, targetId, visited = new Set()) {
            if (rootIdLocal === targetId)
                return [rootIdLocal];
            if (visited.has(rootIdLocal))
                return null;
     ...
     */
    async searchAndFormat(tabId, query, contextLevels = 1, options) {
     ...
     */
    formatSearchResults(snapshotText, searchResult) {
     ...
     */
    formatNode(node, depth, focusAncestorSet) {
    ...
    getNodeAttributes(node) {
        ...
        return attributes.filter((attribute) => attribute !== undefined);
    }
}
// 导出单例实例
