    ...
    async sendCommand(command, params, timeout = DEFAULT_CDP_TIMEOUT) {
        return new Promise((resolve, reject) => {
            ...
            const timeoutId = setTimeout(() => {
                ...
                reject(new Error(`CDP command '${command}' timed out after ${timeout}ms`));
            }, timeout);
            if (!pendingCommands.has(this.tabId)) {
                pendingCommands.set(this.tabId, new Set());
            }
            pendingCommands.get(this.tabId).add(pendingEntry);
            chrome.debugger.sendCommand({ tabId: this.tabId }, command, params, (result) => {
                ...
                if (chrome.runtime.lastError) {
                    reject(new Error(`Failed to send CDP command '${command}': ${chrome.runtime.lastError.message}`));
                }
                else {
                    resolve(result);
                }
