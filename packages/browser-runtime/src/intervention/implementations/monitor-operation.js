...
const metadata = {
    ...
    description: "Monitor user click operations on page, get clicked element info and context",
    enabled: true,
    inputSchema: {
        type: "object",
        properties: {
...
async function execute(params, signal) {
    ...
    return new Promise((resolve, reject) => {
        ...
        elementCaptureService
            .startCapture({
            tabId,
            highlightColor: typeof params === "object" &&
                params !== null &&
            ...
            if (!resolved) {
                ...
                if (errorMessage.includes("browser internal pages")) {
                    reject(new Error("Cannot monitor operations on browser internal pages (chrome://, edge://, etc). Please navigate to a regular webpage first."));
                }
                else if (errorMessage.includes("refresh the page")) {
                    reject(new Error("Cannot connect to the page. The page needs to be refreshed. Please ask the user to refresh the page and try again."));
                }
                else {
                    reject(error);
                }
