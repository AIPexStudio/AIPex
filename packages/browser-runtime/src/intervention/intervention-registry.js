...
export class InterventionRegistry {
    ...
    async initialize() {
        ...
        try {
            const { monitorOperationIntervention } = await import("./implementations/monitor-operation.js");
            this.register(monitorOperationIntervention);
            console.log("✅ [InterventionRegistry] Registered monitor-operation");
            const { voiceInputIntervention } = await import("./implementations/voice-input.js");
            this.register(voiceInputIntervention);
            console.log("✅ [InterventionRegistry] Registered voice-input");
            const { userSelectionIntervention } = await import("./implementations/user-selection.js");
    ...
    register(implementation) {
        ...
        if (this.interventions.has(metadata.type)) {
            console.warn(`[InterventionRegistry] Intervention '${metadata.type}' already registered, overwriting`);
        }
        this.interventions.set(metadata.type, implementation);
        console.log(`[InterventionRegistry] Registered intervention: ${metadata.name}`);
    }
    /**
    ...
    getAllMetadata(enabledOnly = false) {
        const metadataList = Array.from(this.interventions.values()).map((impl) => impl.metadata);
