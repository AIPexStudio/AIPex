...
export class InterventionManager {
    ...
    async requestIntervention(type, params, timeout = 300, reason) {
        ...
        if (this.currentConversationMode === "disabled") {
            console.warn("[InterventionManager] Intervention request rejected: mode is disabled");
            return {
                success: false,
        ...
        console.log(`[InterventionManager] New intervention request:`, request.id, type);
        // If there's an ongoing intervention, queue this one
        if (this.currentIntervention) {
            console.log("[InterventionManager] Queue intervention request (another is active)");
    ...
    async executeIntervention(request) {
        ...
        try {
            ...
            console.log(`[InterventionManager] Executing intervention: ${request.type}`);
            const data = await interventionRegistry.execute(request.type, request.params, this.abortController.signal);
            // Clear timeout
            ...
            state.result = result;
            state.endTime = Date.now();
            console.log(`✅ [InterventionManager] Intervention completed: ${request.id}`);
            ...
            console.error(`❌ [InterventionManager] Intervention error:`, errorMessage);
            const result = {
                success: false,
                error: errorMessage,
                status: "error",
    ...
    cancelIntervention(id) {
        if (!this.currentIntervention ||
            ...
            console.warn(`[InterventionManager] Cannot cancel intervention ${id}: not current`);
            return false;
        }
    ...
    processNextRequest() {
        ...
        if (this.requestQueue.length > 0) {
            ...
            console.log(`[InterventionManager] Processing next request from queue: ${nextRequest.id}`);
    ...
    setupPageMonitoring() {
        ...
        chrome.tabs.onActivated?.addListener((activeInfo) => {
            if (this.currentIntervention?.tabId) {
                if (activeInfo.tabId !== this.currentIntervention.tabId) {
                    console.log("[InterventionManager] Tab switched, cancelling intervention");
                    this.cancelIntervention(this.currentIntervention.request.id);
                }
            }
        });
        ...
        chrome.tabs.onUpdated?.addListener((tabId, changeInfo) => {
            if (this.currentIntervention &&
                ...
                if (changeInfo.url) {
                    console.log("[InterventionManager] Page navigated, cancelling intervention");
                    this.cancelIntervention(this.currentIntervention.request.id);
                }
            }
        });
        ...
        if (listeners) {
            listeners.forEach((listener) => {
                try {
                    ...
                    console.error(`[InterventionManager] Error in event listener for ${type}:`, error);
                }
            });
        }
    }
