 ...
 */
export async function getWindowsByType(
  type: chrome.windows.WindowType,
): Promise<SimplifiedWindow[]> {
  const windows = await chrome.windows.getAll({ populate: true });
...
export async function arrangeWindowsInGrid(
  ...
  try {
    ...
    const windowHeight = Math.floor(
      screenHeight / Math.ceil(normalWindows.length / columns),
    );
 ...
 * Cascade windows
 */
export async function cascadeWindows(): Promise<{
  success: boolean;
  error?: string;
}> {
  try {
