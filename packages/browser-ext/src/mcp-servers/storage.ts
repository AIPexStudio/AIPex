 ...
 */
export async function getAllStorageKeys(): Promise<{
  success: boolean;
  keys?: string[];
  error?: string;
}> {
  ...
  try {
    ...
    return {
      ...
      settings: {
        ...
        autoOrganize: Boolean(
          settings.autoOrganize === "true" || settings.autoOrganize,
        ),
        notifications: Boolean(
          settings.notifications === "true" || settings.notifications,
        ),
      },
    };
  } catch (error: any) {
 ...
 * Export storage data
 */
export async function exportStorageData(): Promise<{
  success: boolean;
  data?: string;
  error?: string;
}> {
...
export async function importStorageData(
  jsonData: string,
): Promise<{ success: boolean; importedKeys?: string[]; error?: string }> {
  try {
