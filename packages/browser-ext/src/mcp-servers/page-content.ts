  ...
  const results = await chrome.scripting.executeScript({
    ...
    func: () => {
      const links = Array.from(document.querySelectorAll("a[href]"))
        ...
        .filter(
          (link) =>
            link.text && link.href && !link.href.startsWith("javascript:"),
        );
  ...
  const results = await chrome.scripting.executeScript({
    ...
    func: () => {
      ...
      const landmarks = Array.from(
        document.querySelectorAll("[role], main, nav, header, footer, aside"),
      ).map((landmark) => ({
  ...
  const results = await chrome.scripting.executeScript({
    ...
    func: (
      ...
      try {
        ...
        const getHighContrastColors = (baseColor: {
          r: number;
          g: number;
          b: number;
        }): string[] => {
        ...
        const persistMessage = persistHighlight
          ? " (persistent)"
          : ` for ${highlightDuration}ms`;
        const colorInfo = options.color
          ? `using custom color ${highlightColor}`
          : `using auto-detected color ${highlightColor} (contrast optimized)`;
        return {
          success: true,
 ...
 */
export async function highlightTextInline(
  selector: string,
  searchText: string,
  options?: {
...
export async function benchmarkInteractiveElements(): Promise<{
  ...
  const results = await chrome.scripting.executeScript({
    ...
    func: () => {
      ...
      const timeReduction =
        standardResult.executionTime - optimizedResult.executionTime;
