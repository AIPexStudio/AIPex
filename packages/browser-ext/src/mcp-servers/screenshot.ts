  ...
  try {
    ...
    if (
      ...
      return {
        success: false,
        error:
          "Cannot capture browser internal pages (chrome:// or extension:// URLs)",
      };
    }
    ...
    if (errorMessage.includes("cannot be captured")) {
      return {
        success: false,
        error:
          "This page cannot be captured. Try refreshing the page or switching to a different tab.",
      };
    }
 ...
 */
export async function getClipboardImageInfo(): Promise<{
  success: boolean;
  hasImage?: boolean;
  imageType?: string;
  error?: string;
