  ...
  try {
    ...
    return {
      ...
      error:
        error?.message ||
        String(error) ||
        "Failed to get downloads. Check if downloads permission is granted.",
    };
  }
}
...
export async function downloadTextAsMarkdown(
  ...
  try {
    ...
    return {
      success: false,
      error:
        error?.message || String(error) || "Failed to download markdown file",
    };
  }
}
...
export async function downloadChatImages(
  ...
  try {
    console.log("💾 [DEBUG] downloadChatImages starting execution:", {
    ...
    for (const message of messages) {
      ...
      for (const part of message.parts) {
        console.log("🖼️ [DEBUG] Processing part:", {
          type: part.type,
        ...
        if (part.type === "image" && part.imageData) {
          try {
            ...
            console.log("📝 [DEBUG] Preparing download:", {
              filename,
              imageDataLength: part.imageData.length,
            ...
            if (result.success && result.downloadId) {
              ...
              const displayFilename =
                result.finalPath?.split("/").pop() || `${baseFilename}.png`;
