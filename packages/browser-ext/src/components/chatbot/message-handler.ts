...
import { CancellationError, CancellationToken } from "../../lib/cancellation-token";
import type {
  UIContextPart,
  UIFilePart,
  UIMessage,
...
export class MessageHandler {
  ...
  constructor(config: MessageHandlerConfig) {
    ...
    this.aiHost =
      config.initialAiHost || "https://api.deepseek.com/chat/completions";
    ...
    if (eventSubscribers) {
      eventSubscribers.forEach((callback) => {
        try {
          ...
          console.error(
            `[MessageHandler] Error in subscriber for event "${event}":`,
            e,
          );
        }
      });
        ...
        if (lastMessage.role === "user" || lastMessage.role === "tool") {
          ...
          if (lastMessage.role === "user") {
            const hostValidation =
              await this._validateHostsInUserMessage(lastMessage);
                  ...
                  parts: [
                    {
                      type: "text",
                      text:
                        hostValidation.message ||
                        "Access denied due to host restrictions.",
                    },
                  ],
                },
              ]);
        ...
        if (lastMessage.role === "assistant") {
          const pendingTools = lastMessage.parts.filter(
            (part): part is UIToolPart =>
              part.type === "tool" && part.state === "input-available",
          );
          ...
          if (pendingTools.length > 0) {
            ...
            console.log(
              "[MessageHandler] Tools execution completed, continuing loop",
            );
          ...
          const allToolParts = lastMessage.parts.filter(
            (part) => part.type === "tool",
          );
          ...
          if (
            ...
            allToolParts.length === completedToolParts.length
          ) {
            console.log(
              "[MessageHandler] All tools completed, sending results back to LLM",
            );
    ...
    for (let msgIndex = 0; msgIndex < history.length; msgIndex++) {
      ...
      if (msg.role === "assistant") {
        ...
        const textParts = msg.parts.filter(
          (p): p is UITextPart => p.type === "text",
        );
        const toolParts = msg.parts.filter(
          (p): p is UIToolPart => p.type === "tool",
        );
        ...
        const shouldIncludeMessage =
          toolParts.length === 0 || allToolsCompleted;
        ...
        toolParts.forEach((part) => {
          if (part.state === "output-available") {
            messages.push({
              role: "tool",
              tool_call_id:
                part.toolCallId || `call_${msg.id}_${part.toolName}`,
              name: part.toolName,
        ...
        const fileParts = msg.parts.filter(
          (p): p is UIFilePart => p.type === "file",
        );
        const contextParts = msg.parts.filter(
          (p): p is UIContextPart => p.type === "context",
        );
   ...
   */
  private async _validateHostsInUserMessage(
    message: UIMessage,
    ...
    const { hostAccessManager } = await import(
      "~/lib/services/host-access-manager"
    );
    ...
    const indirectAccessCheck = await this._checkForIndirectAccess(
      textContent,
      mentionedHosts,
    );
   ...
   */
  private async _checkForIndirectAccess(
    textContent: string,
    mentionedHosts: string[],
  ): Promise<{ allowed: boolean; message?: string }> {
  ...
  private _normalizeUrl(url: string): string | null {
    try {
      ...
      // biome-ignore lint/suspicious/noControlCharactersInRegex: Checking for non-ASCII characters in domain names
      if (hostname.includes("xn--") || hostname.match(/[^\u0000-\u007F]/)) {
        try {
            ...
            try {
              ...
              if (toolCalls) {
                ...
                for (const toolCall of toolCalls) {
                  ...
                  let pendingCall = pendingToolCalls.find(
                  ...
                  try {
                    ...
                    this.setMessages((prev) => {
                      ...
                      let toolPart = assistantMsg.parts.find(
                        (part): part is UIToolPart =>
                          part.type === "tool" &&
                          part.toolCallId === pendingCall.id,
                      );
        ...
        if (assistantMsg) {
          toolsToRun.forEach((toolToRun) => {
            const toolPart = assistantMsg.parts.find(
              (part): part is UIToolPart =>
                part.type === "tool" &&
                part.toolCallId === toolToRun.toolCallId,
            );
            if (toolPart) {
      ...
      const toolResults = await Promise.all(
        toolsToRun.map(async (tool) => {
          try {
            ...
            const isActionTool = await browserMcpClient.checkIsActionTool(
              tool.toolName,
            );
            if (isActionTool) {
