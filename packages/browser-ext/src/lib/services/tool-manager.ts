...
export class ToolManager {
  ...
  public async callTool(toolName: string, args: any, messageId?: string) {
    return await browserMcpClient.callToolWithScreenshot(
      toolName,
      args,
      messageId,
    );
...
export const callTool = (toolName: string, args: any, messageId?: string) =>
  toolManager.callTool(toolName, args, messageId);
export const getToolDescription = (toolName: string) =>
  toolManager.getToolDescription(toolName);
export const hasTool = (toolName: string) => toolManager.hasTool(toolName);
export const getToolCategory = (toolName: string) =>
  toolManager.getToolCategory(toolName);
export const getToolInputSchema = (toolName: string) =>
  toolManager.getToolInputSchema(toolName);
export const getMcpTools = () => toolManager.getMcpTools();
export const getMcpToolDescriptions = () =>
  toolManager.getMcpToolDescriptions();
