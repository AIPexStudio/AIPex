...
const StreamingToolCall: React.FC<StreamingToolCallProps> = ({
  ...
  const [currentStep, setCurrentStep] = useState<StreamingToolCallStep | null>(
    null,
  );
  ...
  const getStepColor = (
    type: StreamingToolCallStep["type"],
    status?: string,
  ) => {
    const baseColors = {
                  ...
                  <div className="flex items-center mb-1">
                    ...
                    <span className="text-sm font-medium text-gray-900">
                      {step.type === "text"
                        ? "Text Response"
                        : step.type === "tool_call"
                          ? `Tool: ${translatedToolName(t, step.name || "")}`
