...
import { useLanguageChanger, useTranslation } from "~/lib/i18n/hooks";
import type { Language } from "~/lib/i18n/types";
import {
  type HostAccessMode,
  hostAccessManager,
} from "~/lib/services/host-access-manager";
...
const ChatBot = () => {
  ...
  const [aiHost, setAiHost, isLoadingHost] = useStorage(
    "aiHost",
    import.meta.env.VITE_AI_HOST ||
      "https://api.openai.com/v1/chat/completions",
  );
  const [aiToken, setAiToken, isLoadingToken] = useStorage(
    "aiToken",
    import.meta.env.VITE_AI_TOKEN,
  );
  const [aiModel, setAiModel, isLoadingModel] = useStorage(
    "aiModel",
  ...
  const [hostAccessMode, setHostAccessMode] =
    useState<HostAccessMode>("include-all");
  ...
  useEffect(() => {
    ...
    const unsubscribeQueue = messageHandlerRef.current.subscribe(
      "queue_changed",
      (newQueue) => {
  ...
  useEffect(() => {
    return () => {
      for (const fn of unsubscribeFunctionsRef.current) {
        fn();
      }
      unsubscribeFunctionsRef.current = [];
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      // Save AI settings
      await // Save AI settings
      setAiHost(tempAiHost);
      await setAiToken(tempAiToken);
      await setAiModel(tempAiModel);
          ...
          <div className="space-y-4">
            ...
            {activeTab === "general" && (
              <div className="space-y-4 py-4">
                ...
                {/* AI Token */}
                <div className="space-y-2">
                  <label className="text-sm font-medium" htmlFor="aiToken">
                    {t("settings.aiToken")}
                  </label>
                  <Input
                    id="aiToken"
            ...
            {activeTab === "security" && (
              <div className="space-y-4 py-4">
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <label
                      htmlFor="host-access-mode-select"
                      className="text-sm font-medium"
                    >
                      Host Access Mode
                    </label>
                    <Select
                      ...
                      onValueChange={(value) =>
                        setHostAccessMode(value as HostAccessMode)
                      }
                    >
                      <SelectTrigger
                        id="host-access-mode-select"
                        className="w-full"
                      >
                        <SelectValue />
                      </SelectTrigger>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {hostAccessMode === "include-all" &&
                        "AI can access any website"}
                      {hostAccessMode === "whitelist" &&
                        "AI can only access whitelisted hosts"}
                      {hostAccessMode === "blocklist" &&
                        "AI cannot access blocklisted hosts"}
                    </p>
                  </div>
