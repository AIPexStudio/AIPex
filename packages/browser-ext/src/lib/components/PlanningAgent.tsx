...
const PlanningAgent: React.FC<PlanningAgentProps> = ({ steps, isActive }) => {
    ...
    <div className="mb-4 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border border-blue-200">
      ...
      <div className="space-y-3">
        {steps.map((step, index) => {
          const stepKey = `${step.type}-${step.timestamp}-${index}`;
          return (
            <div
              key={stepKey}
                      ...
                      {step.toolCall.result && (
                        <div className="text-xs text-green-600">
                          ...
                          {typeof step.toolCall.result === "string"
                            ? step.toolCall.result.slice(0, 200) +
                              (step.toolCall.result.length > 200 ? "..." : "")
                            : `${JSON.stringify(
                                step.toolCall.result,
                                null,
                                2,
                  ...
                  {step.retry && (
                    <div className="mt-2 p-2 bg-cyan-50 rounded border border-cyan-200">
                      <div className="text-xs font-semibold text-cyan-700 mb-1">
                        Retry Attempt {step.retry.attempt}/
