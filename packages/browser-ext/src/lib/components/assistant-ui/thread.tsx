...
export const Thread: FC = () => {
  ...
  const [tabSelectorPosition, setTabSelectorPosition] = useState({
    top: 0,
    left: 0,
  });
  ...
  const fetchCurrentTab = useCallback(async () => {
    try {
      ...
      if (response.success && response.tab) {
        ...
        console.log(
          "🔄 [DEBUG] Adding current tab to selectedTabs:",
          currentTab,
        );
  ...
  const validateAndDeduplicateMessages = useCallback(
      ...
      for (const msg of messages) {
        ...
        if (msg.role === "user") {
          if (lastUserMessage && lastUserMessage.content === msg.content) {
            console.log(
              "🔄 [DEBUG] Skipping duplicate user message:",
              msg.content,
            );
            continue;
          }
  ...
  useEffect(() => {
    if (messages.length > 0) {
      ...
      chrome.storage?.local?.set({
        aipex_conversation_history: deduplicatedMessages,
      });
    }
  ...
  useEffect(() => {
    chrome.storage?.local?.get(["aipex_conversation_history"], (result) => {
      if (
        result?.aipex_conversation_history &&
        Array.isArray(result.aipex_conversation_history)
      ) {
        console.log(
          "🔄 [DEBUG] Restoring conversation history:",
          result.aipex_conversation_history.length,
          "messages",
        );
        const deduplicatedMessages = validateAndDeduplicateMessages(
          result.aipex_conversation_history,
        );
        console.log(
  ...
  useEffect(() => {
    const handleClearMessages = async () => {
      ...
      console.log(
        "🛑 [DEBUG] Stopping all ongoing AI chats before clearing messages",
      );
      try {
  ...
  useEffect(() => {
    const handleStreamMessage = (message: any) => {
      ...
      if (message.request === "ai-chat-stream") {
              ...
              msg.id === message.messageId
                ? {
                    ...
                    parts: (() => {
                      ...
                      const existingToolCall = existingParts.find(
                        (part) =>
                          part.type === "tool_call" &&
        ...
        const isInternalStep = ["think", "act", "observe", "reason"].includes(
          stepType,
        );
        ...
        setMessages((prev) =>
          prev.map((msg) =>
            msg.id === message.messageId ? { ...msg, streaming: false } : msg,
          ),
        );
              ...
              ? {
                  ...
                  parts: [
                    ...(msg.parts || []),
                    {
                      ...
                      imageTitle:
                        message.title ||
                        translatedToolName(t, message.toolName || ""),
                    },
                  ],
                }
              : msg,
  ...
  }, [loading, t]); // Remove currentMessageId dependency to prevent listener removal during task execution

  // Handle providing current chat images for AI tools
  useEffect(() => {
    const handleProvideImages = (
      message: any,
      _sender: any,
        ...
        const imagesInChat = messages
          ...
          .map((msg) => ({
            ...
            parts: msg.parts?.filter(
              (part) => part.type === "image" && part.imageData,
            ),
          }));
  ...
  const handleStopAI = useCallback(async () => {
    console.log("🛑 [DEBUG] handleStopAI called with:", {
      currentMessageId,
      loading,
    });
    if (!currentMessageId || !loading) {
      console.log(
        "🛑 [DEBUG] Early return - no currentMessageId or not loading",
      );
      return;
    }
  ...
  const handleSubmit = useCallback(
    async (message: string) => {
      ...
      if (lastUserMessage && lastUserMessage.content === trimmedMessage) {
        console.log(
          "🔄 [DEBUG] Duplicate user message detected, skipping:",
          trimmedMessage,
        );
        return;
      }
      ...
      try {
          ...
          .reduce((acc, msg, index, array) => {
            ...
            if (index > 0 && msg.role === "user") {
              ...
              if (
                ...
                console.log(
                  "🔄 [DEBUG] Removing duplicate user message from context:",
                  msg.content,
                );
        ...
        const userMessageCount = conversationContext.filter(
          (msg) => msg.role === "user",
        ).length;
        ...
        if (userMessageCount >= 2) {
          ...
          console.log(
            `🔄 [DEBUG] Added "continue with the previous result" to user message #${userMessageCount}`,
          );
        }
          ...
          setMessages((prev) =>
            prev.map((msg) =>
              msg.id === aiMessageId
                ? {
                    ...msg,
                    content: `Error: Failed to start AI chat`,
                    streaming: false,
                  }
                : msg,
            ),
  ...
  useEffect(() => {
    ...
    chrome.storage?.local?.get(["aipex_user_input"], (result) => {
      if (result?.aipex_user_input) {
        console.log(
          "🔄 [DEBUG] Auto-filling input from storage:",
          result.aipex_user_input,
        );
        ...
        setTimeout(() => {
          if (!loading) {
            console.log(
              "🔄 [DEBUG] Auto-submitting message:",
              result.aipex_user_input,
            );
            void handleSubmit(result.aipex_user_input);
            chrome.storage.local.remove("aipex_user_input");
          }
        }, 100); // Slightly longer delay to ensure state is stable
      }
    });
  }, [handleSubmit, loading]); // Remove handleSubmit dependency to prevent re-execution
  ...
  const handleTabSelect = (tab: ReferencedTab) => {
    ...
    const remainingAfterAt = atContent
      ? afterAt.substring(atContent[0].length)
      : afterAt;
          ...
          {messages.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-gray-600">
                Choose a quick action or ask anything to get started
              </p>
              ...
              <div className="grid gap-4 sm:grid-cols-2 mt-8">
                ...
                >
                  ...
                  <div className="text-xs text-gray-600">
                    Use AI to group current-window tabs by topic
                  </div>
                </button>
                ...
                <button
                  ...
                  onClick={() =>
                    handleSubmit(
                      "Summarize this page and save key points to clipboard",
                    )
                  }
                  className="w-full text-left p-6 rounded-2xl border border-blue-200 hover:border-blue-300 hover:bg-white hover:shadow-md bg-white/70 backdrop-blur-sm transition-all duration-200"
                >
                  ...
                  <div className="text-xs text-gray-600">
                    Extract content, summarize, and copy to clipboard
                  </div>
                </button>
                ...
                <button
                  type="button"
                  onClick={() =>
                    handleSubmit("Please use Google to research topic 'MCP'")
                  }
                ...
                >
                  ...
                  <div className="text-xs text-gray-600">
                    Use Google to research and gather information
                  </div>
                </button>
                ...
                >
                  ...
                  <div className="text-xs text-gray-600">
                    Compare product prices across different sources
                  </div>
                </button>
              </div>
            </div>
                                  ...
                                  if (part.imageData) {
                                    ...
                                    if (newTab) {
                                      newTab.document.write(
                                        `<img src="${part.imageData}" style="max-width: 100%; height: auto;" />`,
                                      );
                                    }
                                  }
                                }}
              ...
              <div className="flex flex-wrap gap-2">
                {selectedTabs.map((tab) => (
                  ...
                  >
                    ...
                    <span
                      className="text-gray-900 font-medium max-w-40 truncate"
                      title={tab.title}
                    >
                      {tab.title}
                    </span>
                ...
                >
                  <div className="flex items-center gap-1.5">
                    <div className="w-2.5 h-2.5 bg-blue-100 rounded flex items-center justify-center flex-shrink-0">
                      <svg
                        className="w-1 h-1 text-blue-600"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                        role="img"
