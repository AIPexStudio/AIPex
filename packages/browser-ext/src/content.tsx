...
const Omni = ({
  ...
  const handleCommandSelect = React.useCallback((command: string) => {
    ...
    if (command === "/bookmarks") {
      ...
      chrome.runtime.sendMessage({ request: "get-actions" }, (response) => {
        if (response?.actions) {
          const organizeAction = response.actions.find(
            (a: any) => a.action === "organize-tabs",
          );
          const ungroupAction = response.actions.find(
            (a: any) => a.action === "ungroup-tabs",
          );
  ...
  React.useEffect(() => {
    ...
    if (input.startsWith("/tabs")) {
      chrome.runtime.sendMessage({ request: "get-actions" }, (response) => {
        if (response?.actions) {
          const tabActions = response.actions.filter(
            (a: any) => a.type === "tab",
          );
    ...
    if (
      JSON.stringify(newFilteredActions) !== JSON.stringify(filteredActions)
    ) {
      setFilteredActions(newFilteredActions);
    }
  }, [input, actions, fetchActions, filteredActions]);
  ...
  // Helper functions
  const addhttp = React.useCallback((url: string) => {
  ...
  const handleAction = React.useCallback(
    (action: any) => {
      ...
      switch (action.action) {
        case "google-search":
          window.open(
            `https://www.google.com/search?q=${encodeURIComponent(action.desc)}`,
            "_blank",
          );
          break;
        case "bookmark":
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Escape") {
        ...
        if (showCommandSuggestions) {
          if (e.key === "ArrowDown") {
            setCommandSuggestionIndex((idx) =>
              Math.min(idx + 1, commandSuggestions.length - 1),
            );
          } else {
        ...
        if (showCommandSuggestions) {
          handleCommandSelect(
            commandSuggestions[commandSuggestionIndex].command,
          );
    ...
    return parts.map((part, index) => {
      ...
      if (part.toLowerCase() === searchTerm.toLowerCase()) {
        return (
          <mark
            key={key}
            className="bg-red-100 text-red-900 font-medium px-0.5 rounded"
          >
            {part}
          </mark>
        );
  ...
  return (
    ...
    >
      {/* biome-ignore lint/a11y/noStaticElementInteractions: This div stops propagation to prevent closing the modal */}
      {/* biome-ignore lint/a11y/useKeyWithClickEvents: Keyboard handling is on the parent dialog element */}
      <div
      ...
      >
              ...
              {commandSuggestions.map((suggestion, idx) => (
                ...
                >
                  <div className="font-semibold text-sm">
                    {highlightText(suggestion.command, input)}
                  </div>
                  <div className="text-xs text-gray-500">
                    {highlightText(suggestion.desc, input)}
                  </div>
                </div>
              ))}
            </div>
        ...
        >
          {filteredActions.length === 0 && (
            <div className="text-gray-500 text-lg px-4 py-3">No actions</div>
          )}
          {filteredActions.map((action, idx) => (
            <div
              key={action.id || action.url || `${action.title}-${idx}`}
...
const ContentApp = () => {
  ...
  return (
    <>
      {isOmniOpen && (
        <Omni isOpen={isOmniOpen} onClose={() => setIsOmniOpen(false)} />
      )}
    </>
  );
};
