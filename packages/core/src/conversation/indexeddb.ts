...
export class IndexedDBSessionStorage implements SessionStorageAdapter {
  private storage: IndexedDBStorage<SerializedSession>;

  constructor(dbName = "aipex_sessions") {
    this.storage = new IndexedDBStorage<SerializedSession>({
      dbName,
      storeName: "sessions",
      indexes: [
        { name: "parentSessionId", keyPath: "parentSessionId" },
        { name: "createdAt", keyPath: "metadata.createdAt" },
        { name: "lastActiveAt", keyPath: "metadata.lastActiveAt" },
      ],
    });
  }
